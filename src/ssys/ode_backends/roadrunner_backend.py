...
def _expand_func_calls(text: str, func_name: str, param: str, body: str) -> str:
    ...
    while i < len(text):
        ...
        expanded_body = re.sub(r"\b" + re.escape(param) + r"\b", f"({arg})", body)
...
def _expand_parametric_functions(text: str) -> str:
    ...
    func_def_pattern = re.compile(
        r"^\s*([a-zA-Z_]\w*)\s*\(\s*([a-zA-Z_]\w*)\s*\)\s*:=\s*(.+?)\s*;?\s*$", re.MULTILINE
    )
        ...
        if "'" in line or ("=" in line and ":=" not in line):
            ...
            for _ in range(max_iterations):
                ...
                for func_name, (param, body) in functions.items():
                    ...
                    new_expanded = _expand_func_calls(expanded, func_name, param, body)
