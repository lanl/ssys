...
To assess applicability to real-world models, we applied `ssys` to ODE models from the BioModels database [@Malik2020]. The benchmark suite (included in the repository under `biomodels_batch/`) operates in three phases: (1) fetch SBML models from BioModels, filtering for ODE-based models; (2) identify recast candidates by applying filters that exclude models with unsupported features (discrete events, delay equations) or excessive complexity (species or reaction counts above specified thresholds); and (3) batch recast the candidates. After filtering approximately 1,700 ODE models from BioModels, 937 candidates remained. Of these, `ssys` successfully recast 788 (84%), with all successful recasts passing validation. Most failures were due to processing timeouts (greater than 120 s per model). Benchmark scripts are in `biomodels_batch/` and results are in `biomodels_batch/results/`.
