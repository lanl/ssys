...
- **SBML-first parsing.** Antimony models are converted to SBML with the Antimony library and then parsed with libSBML; SBML files can be parsed directly with `ssys.parse_sbml`. Both paths produce SymPy [@Meurer2017SymPy] symbolic expressions for manipulation.
...
- **Validation.** Three independent tests verify correctness: symbolic Jacobian chain-rule verification, numerical sampling at random positive points, and trajectory comparison via libRoadRunner simulation [@Somogyi2015libRoadRunner; @Welsh2023libRoadRunner].
